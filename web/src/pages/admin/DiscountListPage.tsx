...
import { required, expressionFormat, clearError, validateForm, type FieldErrors } from '../../utils/validate';
    ...
    <div>
          ...
          <tbody>
            {(!data || data.length === 0) ? (
              ...
              <tr key={d.discount_id} className="border-b border-[#EAEAEA] last:border-0">
                ...
                <td className="px-4 py-3 text-xs text-[var(--color-text-secondary)] max-w-[200px] truncate" title={d.description}>{d.description || '-'}</td>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={editing ? '编辑折扣' : '新建折扣'}>
        <div className="flex flex-col gap-4">
          <Input label="折扣 ID" value={form.discount_id} onChange={e => { setForm({ ...form, discount_id: e.target.value }); setErrors(prev => clearError(prev, 'discount_id')); }} disabled={!!editing} error={errors.discount_id} />
          ...
          <Select label="折扣类型" options={PLAN_OPTS} value={form.plan_type} onChange={e => { setForm({ ...form, plan_type: e.target.value }); setErrors(prev => clearError(prev, 'expression')); }} />
          <Input label="表达式" value={form.expression} onChange={e => { setForm({ ...form, expression: e.target.value }); setErrors(prev => clearError(prev, 'expression')); }} hint="ZJ=20, MJ=100-20, ZK=0.8, N=9.99" error={errors.expression} />
          <Select label="分类" options={[{ value: 0, label: '基础折扣' }, { value: 1, label: '人群标签折扣' }]} value={form.discount_type} onChange={e => { setForm({ ...form, discount_type: Number(e.target.value) }); setErrors(prev => clearError(prev, 'tag_id')); }} />
          <Input label="人群标签 ID" value={form.tag_id} onChange={e => { setForm({ ...form, tag_id: e.target.value }); setErrors(prev => clearError(prev, 'tag_id')); }} placeholder="仅人群标签折扣时需要" error={errors.tag_id} />
        </div>
