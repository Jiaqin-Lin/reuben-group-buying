...
// TrialCacheTTL 试算结果缓存 TTL（秒）。短 TTL 防重试穿透，长 TTL 提高缓存命中。
var TrialCacheTTL = NewDef("trial.cache_ttl", 10, "试算结果缓存TTL（秒）")
...
// FeatureSkipPayment 降级开关：跳过支付（测试/压测用）。跳过真实支付网关调用。
var FeatureSkipPayment = NewDef("feature.skip_payment", false, "降级：跳过支付（测试用）")
