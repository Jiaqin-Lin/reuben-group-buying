...
// FeatureUseMockPayment 开关：使用 Mock 支付网关替代真实支付宝（测试用）。
// 与 FeatureSkipPayment 不同，此开关在支付流程完整走通的前提下切换 Mock/Real，
// 锁单、支付、结算全链路可用 Mock 数据验证。
var FeatureUseMockPayment = NewDef("feature.use_mock_payment", false, "开关：使用 Mock 支付网关（测试用）")
